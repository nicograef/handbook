...
| Section | Says |
| --- | --- |
| Where you work | the worktree path and branch; every command runs there; a bare `uv run` or psql reaches the main checkout's store unless the lane's port is prefixed; the data directory's symlinks and real directories; the repo rules that bind (language, no counts in prose, current state only, Conventional Commits, no AI attribution, stage by path) |
...
| Migrations | the reserved numbers by phase; the hard-coded migration list and the grant map to extend; a number nobody reserved is taken as the next free one and reported |
...
- Lane path, branch, cluster and service ports, what the store carries and how it was verified.
