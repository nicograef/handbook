...
- Every subagent dispatch sets `model: opus` (`sonnet` for mechanical work), reviewers included. A usage-limit error means stop and name the reset time. The user switches account; the model never changes.
...
5. Spawn one `general-purpose` agent per lane on `opus` in one message. The whole prompt is: read common.md, read the brief, do the work, return the report. Announce the lanes on the bus with paths and ports.
6. Check in every 30 minutes through a wakeup. Read the agents' last tool calls (`scripts/check-agents.sh`), the bus inbox, `free -m`, the lane units and the base's log. Act on what changes a lane's next step; otherwise do nothing.
...
10. After the wave: status rows flipped, every lane's worktree, branch and cluster gone, the bus told, memory updated. Run `prog compact` before the next wave so the next session starts from a resume file.
