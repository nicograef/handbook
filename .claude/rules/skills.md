...
- Frontmatter: `name` matches the directory; `description` is third person, states what and when, ≤ 250 characters. Optional: `argument-hint`, `allowed-tools`, and `disable-model-invocation: true` for skills with side effects the model should not trigger on its own.
