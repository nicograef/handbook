...
Prove the whole handbook path works on a **throwaway box**.

- Path: provision → harden → deploy with real TLS → monitor → back up →
  restore → patch → tear down.
- Every step runs the owning guide's own **Verify** block against a live server.
- Drift between the docs and reality shows up here, not in production.

> **Operator-performed, not automatable.** Every step that creates an external
> resource is done by a human with the relevant account.
> It cannot run in CI.
> Such resources: Hetzner server, DNS record, Better Stack monitor, real Let's
> Encrypt cert.
...
- **Cost:** a Hetzner **CX23** (2 shared vCPU, 4 GB RAM) plus its IPv4 address.
  - **Well under one euro** for the whole drill.
  - Complete the [teardown](#step-10--teardown) so nothing keeps billing.
...
1. Fill [`templates/cloud-init.yml`](../templates/cloud-init.yml) — set
   `<ssh-public-key>`, `<username>`, `<user-password>`.
2. Point `HEALTH_PING_URL` at the `health` heartbeat you create in
   [Step 6](#step-6--monitoring-and-dead-man-alerting).
   - Leave it as a placeholder for now.
   - You re-run this step's Verify once it is filled.
3. Create the server following the **primary cloud-init path** in
...
4. Wait for cloud-init to finish (`ssh <username>@<host> "sudo cloud-init status --wait"` → `status: done`).
...
Point it at the server's public IPv4, then wait for it to resolve:

```bash
dig +short drill.<your-domain>   # → the server's IP
```

- The Let's Encrypt HTTP-01 challenge in Step 4 fails until this resolves.
- Do not proceed until `dig` returns the box's IP.

## Step 4 — Deploy the drill stack with real TLS

The drill exercises **the handbook's deploy path**, not any application.
It substitutes **stock images for the two `build:` services** in
[`templates/docker-compose.prod.yml`](../templates/docker-compose.prod.yml) — **`backend`** and **`frontend`**.

- Both are normally built from local Dockerfiles.
- Replace each `build:` block with a stock image that satisfies the reverse
...
- **`backend`** → an nginx configured to listen on **8080** (the proxy's `/api/*`
  ...
  - No `depends_on` app build.
  - Keep the `postgres`, `reverse-proxy`, and `certbot` services from the
    template unchanged.
  - The drill still exercises the real DB, proxy, and renewal loop.

Deploy for **real TLS issuance** with [`scripts/prod-init.sh`](../scripts/prod-init.sh).

- It requests a live Let's Encrypt cert and starts the full stack.
- Stage the nginx configs and `.env` first — see
...
> and `/*` → `frontend:80`.
> The stock stand-ins above satisfy both upstreams:
> `https://drill.<your-domain>` returns the frontend nginx welcome page;
...
- Confirm `live/drill.<your-domain>/` exists in the `letsencrypt` volume.
- Confirm the staging `renew --dry-run` prints *"Congratulations, all simulated
  renewals succeeded"*.
- This proves the challenge → issuance → renewal path works end-to-end on a real
  cert.
...
1. Create the **HTTPS uptime monitor** (with the SSL/TLS-expiry alert) on
   `https://drill.<your-domain>`.
   - Follow [monitoring.md → Step 2](monitoring.md#step-2--https-uptime-monitor-with-ssl-expiry-alert).
   - Confirm here whether the free-plan SSL-expiry toggle is actually available
     on your account.
   - That is the caveat that step calls out.
2. Create the **three heartbeats** — `backup`, `cert-renewal`, `health` — per
   [monitoring.md → Steps 3–5](monitoring.md#step-3--backup-heartbeat).
   - Wire `HEALTH_PING_URL` into cloud-init / `/etc/default/report-health`.
   - Re-run [Step 2](#step-2--verify-provisioning)'s health-ping check after.
   - Wire `BACKUP_PING_URL` / `CERT_PING_URL` into the server's Compose `.env`.
3. Run the [monitoring.md → Verify](monitoring.md#verify) block.
   - Fire each heartbeat by hand; confirm all four monitors flip to **up**.
4. **Prove dead-man alerting fires.** Deliberately **miss one ping window** on
   the **`health`** heartbeat.
   - Stop pinging it: temporarily unset `HEALTH_PING_URL` in
     `/etc/default/report-health`, or just do not run the cron.
   - Wait past its **expected period + grace**: 1 day + the 2–3 h grace.
   - That grace comes from [monitoring.md → Step 5](monitoring.md#step-5--health-ping-heartbeat).
   - Confirm Better Stack sends the alert.
   - Restore the URL afterward so the heartbeat recovers to **up**.
...
2. **Exercise the failure mode.** The script verifies each dump with
   `pg_restore --list`.
   - It pings **only after** verification succeeds.
   - Simulate a corrupt archive by **truncating a copy of the dump** so
     verification fails.
   - Confirm the script exits **non-zero** and sends **no ping**:
   ...
   - The `pg_restore --list` on the truncated copy must exit non-zero.
   - This is the exact check inside the script that gates the ping.
   - A run that hits this path leaves **no new verified dump**.
   - The `backup` heartbeat does **not** receive a ping.
   - A missed window would therefore alert, as designed.

## Step 8 — Restore drill

Walk the [postgresql-operations.md → Restore drill](postgresql-operations.md#4-restore-drill)
checklist against the verified dump from Step 7.

- Restore into a **throwaway database**.
- Spot-check row counts, record the outcome, and drop the throwaway DB.
- The drill stack has no app tables, so verify against a table you seed by hand.
- Or confirm the restore completes cleanly with the expected schema.
...
2. **Simulate a pending reboot** and confirm `report-health` **withholds** the
   ...
   - With the ping withheld, the `health` heartbeat misses its next window.
   - Better Stack alerts **after the grace period**.
   - The [reboot routine](maintenance.md#reboot-routine-monthly) relies on the
     same mechanism.
   - It pages you when a kernel patch needs a reboot.
   - Confirm the alert fires.
   - Re-run `report-health` on the (now clean) box; confirm it pings and the
     heartbeat recovers.
...
- [ ] **Delete the drill monitors** in Better Stack — the uptime monitor and all
      three heartbeats.
      - They stop counting against the 10-slot pool.
      - They stop alerting on the now-dead box.
...
- **Every defect found is fixed in the repo and the affected step re-run.**
- The drill ends only when every linked Verify block has passed on the throwaway
  box.
